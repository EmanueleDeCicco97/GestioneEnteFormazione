...
<body>
    ...
    <form action="/modificaPartecipazioni" method="post">
    <h2>Modifica Partecipazione</h2>
        <label for="nIscrizione">Scegli Il Numero Iscrizione:</label>
        <select name="nIscrizione" id="nIscrizione">
            <% List<Partecipazione> partecipazioni2 = (ArrayList) request.getAttribute("partecipazioni");
            for (Partecipazione p : partecipazioni2) { %>
            <option value="<%= p.getnIscrizione()%>"><%= p.getnIscrizione()+" "+ p.getCorsista()+" "+ p.getCorso()%></option>
            <% } %>
        </select><br><br>
        
        <label for="corsi">Scegli Corso:</label>
        <select name="corsi" id="corsi">
            <% List<Corso> corso = (ArrayList) request.getAttribute("corsi");
            for (Corso c : corso) { %>
