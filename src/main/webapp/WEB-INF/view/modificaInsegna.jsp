...
<body>
	...
	<form action="/modificaInsegna" method="post">
		<h2>Modifica Insegnamento per ID</h2>
		<label for="id_insegnamento">Scegli Corso:</label>
        <select name=id_insegnamento id="id_insegnamento">
            <% List<Insegna> insegnamenti1 = (ArrayList) request.getAttribute("insegnamenti");
            for (Insegna i : insegnamenti1) { %>
            <option value="<%= i.getId_insegnamento() %>"><%= i.getId_insegnamento()+" "+ i.getDocente()+" "+ i.getCorso()%></option>
            <% } %>
        </select><br><br>
		<label for="prezzo_docente">Nuovo Prezzo Docente:</label>
		<input type="text" name="prezzo_docente" id="prezzo_docente" required><br>
		<label for="pagato">Stato Pagamento:</label>
		<input type="checkbox" name="pagato" id="pagato"><br>
